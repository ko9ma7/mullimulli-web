...
> **v4.3 온라인 계정 복구:** 화면에는 온라인으로 표시되지만 가입/로그인이 `서버에서 요청을 처리하지 못했습니다`로 실패한다면 `UPDATE_V4_3.cmd`를 실행하세요. v4.3은 Worker 연결뿐 아니라 D1 테이블/컬럼을 실제 검사하고, 임시 계정으로 가입/로그인까지 자동 검증합니다. 기존 계정·친구·편지는 삭제하지 않습니다.
> **v4.3 중요 변경:** 공개 사이트는 이제 실제 Cloudflare Worker/D1 연결이 없으면 로컬 계정 모드로 조용히 떨어지지 않습니다. 다른 컴퓨터에서도 친구가 보여야 하는 실제 서비스에서는 모든 계정/친구/메시지가 D1을 사용합니다. 또한 누군가 나에게 편지를 보내는 순간 본문은 잠근 채 보낸 사람·전달자·D-Day·도착 예정 시간을 바로 보여줍니다.

> **v4.3 배포 복구:** GitHub 웹 업로드로 `.github/workflows/deploy.yml`이 빠져 Pages가 갱신되지 않는 문제를 해결했습니다. `FIX_GITHUB_PAGES.cmd`를 실행하면 Cloudflare/D1은 건드리지 않고 workflow 권한·파일 업로드·Actions·실제 공개 버전까지 검증합니다.

# 멀리멀리 (MulliMulli)

**느리게, 멀리, 마음을 전하세요.**

> **v4.3 UI 재구성**: 사용자 기준 시안에 맞춰 PC 화면을 `Hero + 우측 프로필`, `받는 사람/메시지 + 전달자 6개`, `D-Day/남은 시간/정확한 도착 예정`, `진행 중 여정 3열` 구조로 다시 만들었습니다. 모바일은 같은 정보를 한 열로 재배치합니다.
>
> **배포 확인 방식도 변경**: GitHub에 파일이 올라간 것과 GitHub Pages가 새 버전으로 배포된 것은 별개입니다. 배포 후 `https://ko9ma7.github.io/mullimulli-web/version.txt`에서 **`MULLIMULLI 4.3.0`**이 보여야 실제 v4.3 페이지가 올라간 것입니다.
...
qa/desktop-v4.3.png
qa/mobile-v4.3.png
qa/qa-report-v4.3.json
qa/fidelity-ledger-v4.3.md
```
