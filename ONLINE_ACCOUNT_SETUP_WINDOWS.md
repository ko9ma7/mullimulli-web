...
## v4.0 Windows 실행기/배포 확인

v3.4에서 `spawnSync npx.cmd EINVAL`이 발생할 수 있었습니다. v4.0 패키지에서도 Node.js가 `npx.cmd`를 직접 실행하지 않습니다. `scripts/run-wrangler.ps1`이 PowerShell에서 `npx.cmd`를 호출하고, Node.js 자동설정기는 PowerShell 프로세스만 실행합니다.
