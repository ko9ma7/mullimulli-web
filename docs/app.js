...
function remainingUntil(ts){
  if(!ts) return '계산 전';
  let mins=Math.max(0,Math.ceil((Number(ts)-Date.now())/60000));
  if(mins<=0) return '도착 완료';
  const totalDays=Math.floor(mins/1440); mins%=1440;
  const hours=Math.floor(mins/60), minutes=mins%60;
  if(totalDays>=365){ const years=Math.floor(totalDays/365), months=Math.floor((totalDays%365)/30); return months?`${years}년 ${months}개월`:`${years}년`; }
  if(totalDays>=60){ const months=Math.floor(totalDays/30), days=totalDays%30; return days?`${months}개월 ${days}일`:`${months}개월`; }
  if(totalDays>0) return `${totalDays}일 ${hours}시간`;
...
function recipientReady(friend){
  if(!friend || !state.me?.updatedAt) return false;
  if(isDemoAdmin(friend)) return true;
  if(API_BASE) return Number.isFinite(state.routePreview?.distanceKm) && state.routePreview?.friendId===friend.id;
  return !!friend.updatedAt;
}
function recipientBlockReason(friend){
  if(!friend) return '먼저 받을 친구를 선택해 주세요.';
  if(!state.me?.updatedAt) return '먼저 내 위치를 업데이트해 주세요.';
  if(isDemoAdmin(friend)) return '';
  if(API_BASE){
    if(state.routePreview?.loading) return '두 사람의 마지막 위치를 확인하고 있습니다.';
    if(state.routePreview?.error) return state.routePreview.error;
    if(!Number.isFinite(state.routePreview?.distanceKm)) return '상대방도 마지막 위치를 한 번 업데이트해야 편지를 보낼 수 있습니다.';
    return '';
  }
  if(!friend.updatedAt) return '상대방도 마지막 위치를 한 번 업데이트해야 편지를 보낼 수 있습니다.';
  return '';
}
...
async function loadRoutePreview(friend){
  ...
  state.routePreview={friendId:friend.id,distanceKm:null,loading:true,checkedAt:Date.now()};
  try{
    const data=await api('/api/route-preview',{method:'POST',body:JSON.stringify({toUserId:friend.id})});
    state.routePreview={friendId:friend.id,distanceKm:Number(data.distanceKm),loading:false,checkedAt:Date.now()};
  }catch(e){ state.routePreview={friendId:friend.id,distanceKm:null,loading:false,error:e.message,checkedAt:Date.now()}; }
}
...
async function checkApiHealth(){
  ...
  try{
    ...
    state.apiHealth=data||{ok:false,code:'INVALID_HEALTH_RESPONSE',error:'상태 응답을 읽지 못했습니다.'};
    if(!res.ok||!data?.ok) return state.apiHealth;
    return data;
  }catch(e){
    state.apiHealth={ok:false,code:'API_UNREACHABLE',error:e?.message||'API 연결 실패'};
...
function renderConnectionError(){
  ...
  const schemaProblem=state.apiHealth?.code==='DB_SCHEMA_INCOMPLETE'||state.apiHealth?.code==='DB_UNAVAILABLE'||state.apiHealth?.code==='DB_BINDING_MISSING';
...
async function migrateLocalAccountToOnline(){
  if(!API_BASE) throw new Error('온라인 서버가 연결되지 않았습니다.');
  const local=demoMe();
  if(!local) throw new Error('옮길 로컬 계정을 찾지 못했습니다.');
  const db=demoDb(), pin=String(db.pins?.[local.handle]||'');
  if(pin.length<6) throw new Error('로컬 계정의 PIN 정보를 찾지 못했습니다. 온라인에서 다시 가입해 주세요.');
  try{
    await signup(local.handle,local.nickname,pin);
  }catch(e){
    if(String(e.message||'').includes('이미 사용 중')) await login(local.handle,pin);
    else throw e;
  }
  try{
    await updateProfile({nickname:local.nickname,bio:local.bio||'',avatar:AVATARS.includes(local.avatar)?local.avatar:'🙂',discoverable:local.discoverable!==false,showLocationAge:local.showLocationAge!==false,allowFriendAdd:local.allowFriendAdd!==false});
  }catch{}
...
function bindAuth(){
  ...
  const migrate=document.querySelector('#migrate-local-account'); if(migrate)migrate.onclick=async()=>{const err=document.querySelector('#migrate-error');err.textContent='';migrate.disabled=true;migrate.textContent='옮기는 중…';try{await migrateLocalAccountToOnline();await afterLogin();toast('로컬 계정을 온라인 계정으로 옮겼습니다.');}catch(ex){err.textContent=ex.message;migrate.disabled=false;migrate.textContent='온라인으로 옮기기';}};
}
...
async function render(){
  ...
  if(state.selectedFriend && API_BASE){
    const rp=state.routePreview, staleError=rp?.error && Date.now()-(rp.checkedAt||0)>5000;
...
function sendPanel(friends,messages=[]){
  const f=state.selectedFriend||friends[0]||(!API_BASE?demoAdmin():null);
  let c=currentCourier();
  const preview=previewDistance(f);
  if(Number.isFinite(preview)&&state.courierFilter==='recommended'&&!courierMatchesDistance(c,preview)){const next=filterCouriers(preview)[0];if(next){state.courier=next.id;c=next;}}
  const est=f?routeEstimate(f,c):{distance:null,hours:null,eta:null};
  ...
  const successRate=100-Math.round(c.fail*100), arrival=est.eta||null;
  const adminTest=!API_BASE?`<div class="demo-recipient compact-demo"><div class="demo-recipient-head"><div><strong>🕊️ 관리자 테스트 우편함</strong><p>친구가 없어도 내 위치 기준 가상 거리로 전송 시간을 시험할 수 있습니다.</p></div><button type="button" class="btn btn-secondary ${isDemoAdmin(f)?'selected-soft':''}" data-select-admin>${isDemoAdmin(f)?'선택됨':'관리자로 테스트'}</button></div>${isDemoAdmin(f)?`<div class="distance-presets" aria-label="관리자 테스트 거리">${DEMO_DISTANCE_PRESETS.map(km=>`<button type="button" class="filter-chip ${state.demoDistanceKm===km?'active':''}" data-demo-distance="${km}">${km<1?`${Math.round(km*1000)}m`:`${km.toLocaleString()}km`}</button>`).join('')}</div>`:''}</div>`:'';
  ...
  const deliverySummary=`<div class="delivery-summary"><div class="selected-courier-summary"><span class="courier-emoji">${c.emoji}</span><div><span class="summary-label">선택한 전달자</span><strong>${escapeHtml(c.name)}</strong><small>성공률 <b>${successRate}%</b> · ${Number.isFinite(distance)?`${distance.toFixed(1)} km`:'거리 확인 전'} · ${fmtDuration(est.hours)}</small></div></div><div class="delivery-time-cell dday-cell"><span class="summary-label">도착까지</span><strong class="dday">${arrival?dDayLabel(arrival):'D-?'}</strong></div><div class="delivery-time-cell"><span class="summary-label">남은 시간</span><strong>${arrival?remainingUntil(arrival):'계산 전'}</strong><small>실제 서비스 시간</small></div><div class="delivery-time-cell arrival-cell"><span class="summary-label">도착 예정</span><strong>${arrival?fmtJourneyDay(arrival):'위치 확인 필요'}</strong><small>${arrival?fmtJourneyClock(arrival):'두 사람 위치 필요'}</small></div><div class="delivery-submit-cell"><button class="btn btn-primary send-main-btn" type="submit" form="send-form" ${recipientReady(f)?'':'disabled'}>✈️ 편지<br>출발시키기</button></div></div>`;
  return `<section class="panel send-workspace"><div class="workspace-head"><div><span class="eyebrow">편지 보내기</span><h2>받는 사람, 메시지, 전달자를 한 번에 정하세요.</h2><p>${f?`${escapeHtml(f.nickname)}에게 `:''}<strong>${distanceText}</strong></p></div><span class="connection-chip ${API_BASE?'is-online':'is-local'}">${API_BASE?'● 온라인 연결':'○ 로컬 데모'}</span></div>${adminTest}<div class="compose-layout"><form id="send-form" class="compose-card"><div class="compose-step"><div class="step-title"><span>1</span><strong>누구에게 보내나요?</strong></div><div class="field"><input id="friend-search" value="${escapeHtml(state.search)}" placeholder="친구 이름 또는 @아이디 검색" autocomplete="off" /></div><div id="friend-results" class="friend-search-results compact-results">${friendRows}</div></div><div class="compose-step message-step"><div class="step-title"><span>2</span><strong>어떤 마음을 전할까요?</strong></div><div class="field"><textarea id="message-body" maxlength="500" placeholder="마음을 담아 메시지를 작성하세요..." required>${escapeHtml(state.messageDraft)}</textarea><div class="compose-tools"><span class="hint">🔒 도착 전에는 열리지 않습니다.</span><span class="char-count"><span id="chars">${state.messageDraft.length}</span>/500</span></div></div></div></form><div class="courier-picker ${state.courierExpanded?'is-expanded':''}"><div class="step-title courier-title"><span>3</span><strong>전달자를 선택하세요.</strong></div>${courierFilterBar()}<div class="courier-row">${courierCards(f)}</div></div></div>${deliverySummary}<div class="immutable-inline bottom-rule"><span>🔒</span><span>출발 후에는 취소하거나 목적지를 바꿀 수 없습니다.</span>${API_BASE&&f?'<button type="button" class="route-refresh" id="refresh-route">↻ 거리 다시 확인</button>':''}</div>${!recipientReady(f)?`<p class="hint send-block-reason">${escapeHtml(recipientBlockReason(f))}</p>`:''}</section>${journeyOverview(messages)}`;
}
function journeyOverview(messages){
  const active=messages.filter(m=>statusFor(m)==='transit').slice(0,3);
  if(!active.length) return '';
  return `<section class="journey-overview"><div class="section-inline-head"><div><span class="eyebrow">현재 진행 중인 여정</span><h2>도착까지 남은 시간을 확인하세요.</h2></div><button type="button" class="text-button" data-tab="journeys">전체 보기 →</button></div><div class="journey-overview-grid">${active.map(m=>journeyCard(m,true,true)).join('')}</div></section>`;
}
...
function bindMain(friends){
  ...
  const toggleCouriers=document.querySelector('#toggle-couriers'); if(toggleCouriers)toggleCouriers.onclick=()=>{state.courierExpanded=!state.courierExpanded;state.courierFilter=state.courierExpanded?'all':'recommended';render();};
  ...
  document.querySelectorAll('[data-select-friend]').forEach(b=>b.onclick=()=>{state.selectedFriend=friends.find(f=>f.id===b.dataset.selectFriend)||state.selectedFriend; state.routePreview=null; state.courierFilter='recommended'; state.courierExpanded=false; state.tab='send';render();});
  const adminButton=document.querySelector('[data-select-admin]'); if(adminButton)adminButton.onclick=()=>{state.selectedFriend=demoAdmin();state.routePreview=null;state.courierFilter='recommended';state.courierExpanded=false;render();};
  ...
  const refreshRoute=document.querySelector('#refresh-route'); if(refreshRoute)refreshRoute.onclick=async()=>{refreshRoute.disabled=true;refreshRoute.textContent='확인 중…';state.routePreview=null;try{await loadRoutePreview(state.selectedFriend);}finally{render();}};
  const body=document.querySelector('#message-body'); if(body)body.oninput=()=>{state.messageDraft=body.value;const chars=document.querySelector('#chars');if(chars)chars.textContent=body.value.length;};
  ...
  if(search) search.oninput=()=>{
    ...
    holder.querySelectorAll('[data-select-friend]').forEach(b=>b.onclick=()=>{state.selectedFriend=rows.find(f=>f.id===b.dataset.selectFriend);state.routePreview=null;state.courierFilter='recommended';state.courierExpanded=false;render();});
  };
